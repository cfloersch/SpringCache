...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    */
   // Guarded By Segment.this
   private ValueReference<K, V> newValueReference(ReferenceEntry<K, V> entry, V value, int weight) {
    ...
    * instead.
    */
   private boolean isLive(ReferenceEntry<K, V> entry, long now) {
   ...
   public V computeIfAbsent(K key, Function<? super K, ? extends V> function) {
      checkNotNull(key);
      checkNotNull(function);
      return compute(key, (k, oldValue) -> (oldValue == null) ? function.apply(k) : oldValue);
   }
