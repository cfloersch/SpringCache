...
import com.xpertss.cache.store.Weighable;
...
class LocalCache<K, V> {
   ...
   /** How long after the item is considered stale the map will retain that item. */
   final long expireAfterStaleMillis;


   // TODO Next 8 need to go
    ...
    * soft/weak entries.
    */
   final CacheListener<K, V> removalListener;

   /** Measures time in a testable way. */
   final TimeProvider ticker;
   ...
   LocalCache(CacheBuilder<? super K, ? super V> builder)
   {
      ...
      expireAfterStaleMillis = builder.getExpireAfterStaleMillis();
      ...
      // TODO Remove
      keyEquivalence = Strength.STRONG.defaultEquivalence();
      valueEquivalence = Strength.STRONG.defaultEquivalence();

      maxWeight = builder.getMaximumWeight();

      // TODO Remove
      weigher = (key, value) -> {
         if(value instanceof Weighable) {
            ((Weighable)value).weigh();
         }
         return 0;
      };

      // TODO Remove
      expireAfterAccessNanos = 0;
      expireAfterWriteNanos = 0;
      refreshNanos = 0;

      removalListener = builder.getRemovalListener();
      removalNotificationQueue = (removalListener == null)
         ? discardingQueue()
         : new ConcurrentLinkedQueue<RemovalNotification<K, V>>();

      ticker = builder.getTimeProvider();
      ...
      if (evictsBySize()) {
         ...
         // TODO This is never going to be used
         for (int i = 0; i < this.segments.length; ++i) {
   ...
   boolean expires() {
      // will always be true (but after stale)
   ...
   ValueReference<K, V> newValueReference(ReferenceEntry<K, V> entry, V value, int weight) {
      ...
      return valueStrength.referenceValue(segmentFor(hash), entry, Objects.notNull(value, "value"), weight);
   }
   ...
   static class Segment<K, V> extends ReentrantLock {
      ...
      //GuardedBy("this")
      ReferenceEntry<K, V> newEntry(K key, int hash, ReferenceEntry<K, V> next) {
         return map.entryFactory.newEntry(this, Objects.notNull(key, "key"), hash, next);
      }
   ...
   public V getIfPresent(Object key) {
      int hash = hash(Objects.notNull(key, "key"));
      return segmentFor(hash).get(key, hash);
   }

   public V put(K key, V value) {
      int hash = hash(Objects.notNull(key, "key"));
      return segmentFor(hash).put(key, hash, Objects.notNull(value, "value"), false);
   }
   ...
   static class LocalManualCache<K, V> implements Cache<K, V>, Serializable {
      ...
      public void evict(Object key) {
         localCache.remove(Objects.notNull(key, "key"));
      }
